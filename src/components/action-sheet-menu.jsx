...
function DevModal({ visible, setDevModal }) {
  ...
  return (
    ...
    >
      <div className={'flex justify-end p-2'} onClick={() => setDevModal(false)}>
      ...
      <div className={'h-full overflow-auto p-2'}>
        ...
        <div className={'my-1'}>
          ...
          <textarea
            defaultValue={
              token ? 'http://localhost:5173/?token=' + token + '&language=' + lang : ''
            }
            rows={textAreaRow}
            className={`w-full border border-black`}
          />
...
export default function ActionSheetMenu() {
  ...
  const buttonsData = [
    ...
    },
    {
      ...
      onClick: () =>
        window.open(
          `https://fga-accounts-center.pages.dev/?token=${token}&&language=${lang}`,
          '_self',
        ),
    },
    {
